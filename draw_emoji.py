...
class EmojiDrawerApp:
	...
	def _build_ui(self) -> None:
		...
		ttk.Label(sidebar, text="Draw an emoji").grid(row=0, column=0, sticky="w")
		...
		ttk.Button(btn_row, text="Clear", command=self.on_clear).grid(row=0, column=0, sticky="ew", padx=6)
		ttk.Button(btn_row, text="Save PNG", command=self.on_save).grid(row=0, column=1, sticky="ew")

		self.guess_var = tk.StringVar(value="—")
		ttk.Label(sidebar, text="Top guess:").grid(row=2, column=0, sticky="w", pady=(12, 0))
		ttk.Label(sidebar, textvariable=self.guess_var, font=("Segoe UI Emoji", 36)).grid(row=3, column=0, sticky="w")

		self.probs_text = tk.Text(sidebar, wrap="none", font=('Segoe UI Emoji', 20), width=12, height=6)
